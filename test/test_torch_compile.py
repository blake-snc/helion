...
class TestTorchCompile(RefEagerTestDisabled, TestCase):
    ...
    def test_epilogue_matmul_with_transposed_self(self, allow_torch_compile_fusion):
        ...
        of mm and once as the right (transposed). Without a guard that checks
        for multiple consumers, fusion incorrectly removes buf1, causing a
    ...
    @skipIfRocm("torch.compile missing kernel metadata on ROCm")
    @skipIfTileIR("torch.compile missing kernel metadata on tileir")
    def test_prologue_flip_not_fused(self, allow_torch_compile_fusion):
        """Prologue that flips the input tensor is not inlined.

        k_add(x.flip(0), x) has x.flip(0) as a prologue with index remapping:
        the prologue reads source[N-1-i, j] while writing out[i, j].  Fusion
        would replace the load with _load_val at the kernel's standard index,
        producing 2*x instead of flip(x)+x.  The guard in
        _prologue_group_has_index_remapping detects this and codegen's the
        prologue as a separate kernel, giving the correct result.
        """
    ...
    @skipIfRocm("torch.compile missing kernel metadata on ROCm")
    @skipIfTileIR("torch.compile missing kernel metadata on tileir")
    def test_epilogue_triu_not_fused(self, allow_torch_compile_fusion):
        """Epilogue using triu (index_expr) is not fused."""
